...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    thinking_msg = await update.message.reply_text("🤔 기억을 더듬으며 생각 중...")
    
    try:
        from scripts.core.orchestrator import orchestrate_agents
        
        context_data = retrieve_context(text)
        final_reply = orchestrate_agents(text, context_data)
        
        if not final_reply:
            final_reply = "오류: 최종 응답을 생성하지 못했습니다."
            
        await thinking_msg.edit_text(final_reply)
    except Exception as e:
        await thinking_msg.edit_text(f"❌ AI 생성 오류 발생:\n{str(e)}")
